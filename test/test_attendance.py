...
def test_swipe_in():
    ...
    db.log_swipe_in("Lucas","Name","Merc","Merc")
    assert db.get_active_users()[0]["id"] == "Lucas"

def test_swipe_in_multi():
    db = CCRAttendance.open_db_interface(res("client_secret.json"),"Node",res("db_config.json"))
    db.clear_attendence_log()
    db.log_swipe_in("Lucas","Name","Merc","Merc")
    db.log_swipe_in("Lucas2","Name","Merc","Merc")
    assert db.get_active_users()[0]["id"] == "Lucas" and db.get_active_users()[1]["id"] == "Lucas2"
